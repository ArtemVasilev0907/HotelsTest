package com.skydivers.hotelstest.ui.design.delegateAdaper
...
open class CompositeDelegateAdapter(vararg adapters: DelegateAdapter) : RecyclerView.Adapter<ViewHolder>() {
    ...
    fun findAdapters(adapter: DelegateAdapter):List<DelegateAdapter>{
        ...
        for(i in 0 until  adapterState.data.size ){
            ...
            if (adapterState.getAdapterByItemPosition(i)::class.simpleName ==  adapter::class.simpleName){
                Log.e(adapterState.getAdapterByItemPosition(i)::class.simpleName,"adapter: ${adapter::class.simpleName}")
    ...
    fun checkRequires( adapter: DelegateAdapter){


        for(i in 0 until  adapterState.data.size ){

            if (adapterState.getAdapterByItemPosition(i)::class.simpleName ==  adapter::class.simpleName){
                adapterState.getAdapterByItemPosition(i).checkRequires()
    ...
    open fun swapData(data: List<Any>) {
        ...
        Log.e(this::class.simpleName,"at: ${data.size}")
    ...
    fun onUserAction(onUserAction: (BookingUserAction) -> Unit) {
